...
namespace oceanbase
{
  namespace sql
  {
    ...
    // RioChen-文件解析部分涉及到的就是以下三个类

    // RioChen-从csv文件里读取数据并存放在buffer里面
    ...
    private:
      ...
      // 1表示预留一个元素的空间(locate_buffer)，直接插入一个，就直接插入到分配的地方，继续插，就继续分配内存。
      //这样做的目的是有的情况下我们知道数组应该分配的最大空间
    ...
    // RioChen-2.列值转换：字符集转换、类型转换、顺序转换
    ...
    public:
      ...
      // RioChen-newrow都是stirng，datumrow是类型转换后的值，列的位置也做了转化
      int get_casted_row(const common::ObNewRow &new_row, const ObLoadDatumRow *&datum_row);

    private:
      // RioChen-在初始化时将column_schemas和column_idxs_建好，即将列结构信息以及数据源的列和存储列对应关系建好
      // RioChen-目的是进行顺序转换，即将每行的列值按照数据库存储序列格式排序：数据库中的每一行数据在存储时，会将主键对应的列移到前面
      int init_column_schemas_and_idxs(
          const share::schema::ObTableSchema *table_schema,
          const common::ObIArray<ObLoadDataStmt::FieldOrVarStruct> &field_or_var_list);
      // RioChen-字符集转化和类型转化，在这里做的
    ...
    private:
      // broke_size,如果内存不够，就分配两倍，然后拷贝
      // RioChen-每一列的结构信息，和存储列是对应的
    ...
    // RioChen-3.（外部）排序
    // RioChen-具体实现在ob_parallel_external_sort.h
    ...
    private:
      // RioChen-初始化sstable_index_builder和macro_block_writer，随后一直调用append_row来将有序数据添加到micro_block_writer里
