...
namespace oceanbase
{
  namespace sql
  {
    ...
    // RioChen-这里进行比较时主要就是比较主键，这是构建一个datum_utils来实现的
    ...
    int ObLoadSSTableWriter::init(const ObTableSchema *table_schema)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          // RioChen-这四行不要动，多加flag，多加两个列（多版本列？）
    ...
    // RioChen-向macro_block_writer里塞数据
    int ObLoadSSTableWriter::append_row(const ObLoadDatumRow &datum_row)
    {
      ...
      else
      {
        for (int64_t i = 0; i < column_count_; ++i)
        {
          ...
          // RioChen-macro_block_writer里加了两个多版本列，加的位置是在主键列的后面，即表里总共16列，但存储时实际上有18个列
          {
    ...
    int ObLoadSSTableWriter::create_sstable()
    {
      ...
      SMART_VAR(ObSSTableMergeRes, merge_res)
      {
        ...
        else
        {
          ...
          // RioChen-使用分配器来创建sstable，这里使用table_handle用来hold住引用计数
          ...
          else
          {
            ...
            // RioChen-把生成的sstable更新到这个表对应的存储队列里面去
    ...
    // RioChen-close掉macro_block_writer：先把数据加入到内存，内存满了之后才会将数据刷入到macro_block中，随后也会生成inndex_builder
    ...
    int ObLoadDataDirectDemo::do_load()
    {
      ...
      // RioChen-到这里，外排已经结束，下面的代码做的是写入sstable
      while (OB_SUCC(ret))
      {
